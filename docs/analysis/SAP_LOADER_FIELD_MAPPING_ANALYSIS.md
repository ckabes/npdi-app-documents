...
## 10. Detailed Implementation To-Do List

### Phase 1: Schema Verification & Minor Enhancements (Priority: Medium)

**Already Implemented in NPDI:**
- [x] `grossWeight` (value/unit) - skuVariantSchema
- [x] `netWeight` (value/unit) - skuVariantSchema
- [x] `volume` (value/unit) - skuVariantSchema
- [x] `primaryPlant` - BaseTicket
- [x] `pricing.standardCost` - skuVariantSchema
- [x] `packageSize` (value/unit) - skuVariantSchema
- [x] **PlantCode** model - Reference data for plant codes
- [x] **WeightMatrix** model - Gross weight lookup table

**Optional Enhancements (Low Priority):**
- [ ] **1.1** Add dimensions fields to skuVariantSchema (if needed)
  - `dimensions.length`, `dimensions.width`, `dimensions.height`

- [ ] **1.2** Add supplier part number to BaseTicket (if needed)
  - `supplierPartNumber` (String)

### Phase 2: Mapping Configuration (Priority: High)

- [ ] **2.1** Create SBU → SAP mapping table
  ```javascript
  // config/sapMappings/sbuMapping.js
  {
    'P79': { spart: '86', ygart: 'SIAL99', ylogo: 'SA', lsweb: 'SI' },
    '635': { spart: '86', ygart: '612525', ylogo: 'MP', lsweb: 'MI' },
    // ...
  }
  ```

- [ ] **2.2** Create quality segment mapping
  ```javascript
  // config/sapMappings/qualityMapping.js
  {
    '100': 'Technical',
    '200': 'Puriss',
    '300': 'ACS/Reagent',
...
- [ ] **2.3** Create temperature condition mapping
  ```javascript
  // config/sapMappings/tempConditionMapping.js
  {
    'Room Temperature': 'W1',
    'Refrigerated': 'K1',
    'Frozen': 'K2',
...
- [ ] **2.4** Create unit of measure mapping
  ```javascript
  // config/sapMappings/uomMapping.js
  {
    'g': 'G', 'kg': 'KG', 'mg': 'MG',
    'mL': 'ML', 'L': 'L',
    'ea': 'EA', 'pk': 'PK'
...
- [ ] **2.5** Create material type determination logic
  - BULK → HALB (semi-finished)
  - SKU → FERT (finished product)

- [ ] **2.6** Document all mapping decisions in config

### Phase 3: Core Export Service (Priority: High)

- [ ] **3.1** Create `SAPLoaderExporter` main service class
  - Input: Ticket ID
  - Output: Excel buffer or file path

- [ ] **3.2** Create `FieldMapper` service
  - Maps NPDI fields to SAP fields
  - Handles direct mappings
  - Handles derived/calculated fields

- [ ] **3.3** Create `EnumMapper` service
  - Transforms NPDI enums to SAP codes
  - Handles missing mappings gracefully

- [ ] **3.4** Create `ValidationService`
  - Validates required SAP fields
  - Generates missing data report
  - Returns warnings for optional missing fields

- [ ] **3.5** Create `ExcelGenerator` service
  - Generates MDG-M sheet
  - Generates Next Classification sheet
  - Generates Next Plant sheet
  - Maintains exact loader format (3-row header)

### Phase 4: Multi-Variant Support (Priority: Medium)

- [ ] **4.1** Generate BULK material row (HALB type)
- [ ] **4.2** Generate SKU variant rows (FERT type)
- [ ] **4.3** Handle variant-specific field differences
- [ ] **4.4** Support SKU naming conventions (e.g., PMC2013-1KG)

### Phase 5: API Endpoints (Priority: Medium)

- [ ] **5.1** Create export endpoint
  ```
  POST /api/tickets/:id/export/sap-loader
  ```

- [ ] **5.2** Create preview endpoint (returns mapping without generating file)
  ```
  GET /api/tickets/:id/export/sap-loader/preview
  ```

- [ ] **5.3** Create validation-only endpoint
  ```
  GET /api/tickets/:id/export/sap-loader/validate
  ```

- [ ] **5.4** Create bulk export endpoint
  ```
  POST /api/tickets/export/sap-loader/bulk
  Body: { ticketIds: [...] }
  ```

### Phase 6: User Interface (Priority: Medium)

- [ ] **6.1** Add "Export to SAP" button on ticket detail page
- [ ] **6.2** Create export preview modal
  - Show mapped fields
  - Highlight missing/required fields
  - Show warnings
- [ ] **6.3** Add missing data form for required fields
- [ ] **6.4** Create export history/audit log view
- [ ] **6.5** Add bulk export selection on ticket list

### Phase 7: Testing (Priority: High)

- [ ] **7.1** Unit tests for FieldMapper
- [ ] **7.2** Unit tests for EnumMapper
- [ ] **7.3** Unit tests for ValidationService
- [ ] **7.4** Integration tests for ExcelGenerator
- [ ] **7.5** End-to-end test with sample chemical product
- [ ] **7.6** Validate generated file format with SAP team
- [ ] **7.7** Test import of generated files into MDG-M

### Phase 8: Documentation (Priority: Low)

- [ ] **8.1** Create user guide for SAP export feature
- [ ] **8.2** Document all field mapping decisions
- [ ] **8.3** Create troubleshooting guide
- [ ] **8.4** Document API endpoints
